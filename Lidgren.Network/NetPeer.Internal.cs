...
public partial class NetPeer
{
		...
		while ((bytesReceived - ptr) >= NetConstants.HeaderByteSize)
		{
			...
			try
			{
				...
				else
				{
					...
					Buffer.BlockCopy(m_receiveBuffer, ptr, msg.BufferData, 0, payloadByteLength);
					if (sender != null)
					{
	...
	internal NetIncomingMessage SetupReadHelperMessage(int ptr, int payloadLength)
	{
		...
		if (m_readHelperMessage == null)
		{
			throw new InvalidOperationException(nameof(m_readHelperMessage) + " is null");
		}
